function EnergyArray = Energy_calc(numVars,fold)
%% to calculate energy 

EnergyArray = cell(1, numVars-1);
counter=1;
    ...
    if file.isdir && ~ismember(file.name, {'.', '..'}) % check if the current file is a subdirectory
        ...
        for j = 1:numel(subfolder_files)
            ...
            if ~subfolder_file.isdir % check if the current file is not a subdirectory
                ...
                [~, subfolder_values] = regexp(subfolder_name, 'pid-(\d+\.\d+)-(\d+\.\d+)-(\d+\.\d+)', 'match', 'tokens');
                subfolder_values = str2double(subfolder_values{1});
                X_value = subfolder_values(1);
                X_value = round(X_value*100);
                varname = sprintf('X%d_%d', X_value, j-2);         % create a unique variable name for each subfolder
                ...
                eval(['data = ' header ';']);
                eval([varname '= data;']);
                EnergyArray{counter} = {varname, data};
                counter = counter + 1;
                clear data;
                if counter == numVars
                    break;

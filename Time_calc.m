function TimeArray = Time_calc(numVars,fold)

%% result time 
    TimeArray = cell(1, numVars-1);
    counter=1;
        ...
        if file.isdir && ~ismember(file.name, {'.', '..'}) % check if the current file is a subdirectory
            ...
            for j = 1:numel(subfolder_files)
                ...
                if ~subfolder_file.isdir % check if the current file is not a subdirectory
                    ...
                    varname = sprintf('N%d_%d', X_value, j-2);         % create a unique variable name for each subfolder
                    ...
                    eval(['data = ' header ';']);
                    eval([varname '= data;']);
                    TimeArray{counter} = {varname, data};
                    counter = counter + 1;
                    clear data;
                    if counter == numVars
                        break;
